...
RESULTS_DIR=$CODEDIR/results/`date +'%y%m%d%H%M%S'`
...
if [ ! -d "${DATA_DIR_PHASE1}" ] || [ -z "$(ls -A ${DATA_DIR_PHASE1})" ]; then
   ...
   preprocess_cmd=" \
      mpirun \
         ...
         --allow-run-as-root \
         -np ${num_dask_workers} \
         -x LD_PRELOAD=/usr/local/lib/libjemalloc.so \
...
if [ "$train_steps" -le "0" ] ; then
   echo "train steps ${train_steps} <= 0, skip this phase"
elif [ -z "$LOGFILE" ] ; then
   $CMD
else
   (
...
set -x
if [ "$train_steps_phase2" -le "0" ] ; then
   echo "train steps ${train_steps_phase2} <= 0, skip this phase"
elif [ -z "$LOGFILE" ] ; then
   $CMD
else
   (
